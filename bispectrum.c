...
periodic=1;
 loop_interlacing_periodic_for_bispectrum(Ninterlacing, s_x, s_y, s_z, weight, Ndata, L1, L2, ngrid_i, n_lines_parallel, type_of_mass_assigment, mode_correction, deltak_re, deltak_im,name_density,type_of_input);
    ...
    if(strcmp(type_of_code,"rusticoX") ==0 ){
        ...
        loop_interlacing_periodic_for_bispectrum(Ninterlacing, s_xB, s_yB, s_zB, weightB, NdataB, L1, L2, ngrid_i, n_lines_parallel, type_of_mass_assigment, mode_correction, deltak_reB, deltak_imB,NULL,type_of_input);
...
void loop_bispectrum_periodic_for_gadget_x_ascii_caller(double kmin,double kmax,int Ninterlacing, double L1, double  L2, int ngrid, double  Deltakbis, int  mode_correction, int n_lines_parallel, char *binning_type, char *name_bsAAA_out, char *name_bsAAB_out, char *name_bsABA_out, char *name_bsBAA_out,char *name_bsABB_out, char *name_bsBAB_out,char *name_bsBBA_out, char *name_bsBBB_out,char *name_bs002_out,char *name_bs020_out,char *name_bs200_out, char *type_of_mass_assigment, char *triangles_num, char *write_triangles, char *triangles_id, char *name_data_in, int gadget_files,  double *s_xB, double *s_yB, double *s_zB, double *weightB, long int NdataB, double Power_spectrum_shot_noiseB,char *do_multigrid, char *bis_opt, char *triangle_shapes,char *RSD,char *do_bispectrum2,char *type_of_code,int reverse, char *type_of_survey)
{
    long int input_bin[1];
    int i,j,ngrid_i;
    long int l;
...
int Nk;
    ...
    Power_spectrum_shot_noiseA=params[0];
     ...
     loop_interlacing_periodic_for_bispectrum(Ninterlacing, s_xB, s_yB, s_zB, weightB, NdataB, L1, L2, ngrid_i, n_lines_parallel, type_of_mass_assigment, mode_correction, deltak_reB, deltak_imB,NULL,name_particles);
        }
